...
import { xpBudget, xpCost, xpSpent, type XpKind, type XpPurchase } from "./experience";
...
export default function XpPurchasePanel(props:Props){
  ...
  const cost=xpCost(kind==="discipline"?disciplineKind:kind,isMagic?magic?.level||0:current+1);
  ...
  return <div className="xp-panel">
    <p>A distribuição inicial permanece separada. Compras registradas aqui não alteram as metas da criação.</p>
    ...
    <label className="field"><span>XP total da ficha, incluindo o inicial</span><input type="number" min={0} value={props.total===""?budget:props.total} onChange={event=>props.onTotal(event.target.value)}/></label>
    <small className="xp-help">{props.ageCategory==="Neófito"?"Neófito: 15 XP iniciais.":props.ageCategory==="Ancilla"?"Ancilla: 35 XP iniciais.":"Esta categoria não recebe XP inicial pela regra-base."} Ajuste o total apenas se a crônica conceder mais XP.</small>
